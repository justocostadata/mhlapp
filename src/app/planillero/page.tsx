...
export default async function PlanilleroPage() {
  ...
  // Fetch only matches assigned to this scorekeeper
  const { data: assignedMatches } = await supabase
    .from("matches")
    .select("id, match_type, scheduled_at, venue_name, pitch, status, player_price, max_players, home_score, away_score")
  ...
  const { data: activePlayers } = matchIds.length > 0
    ? await supabase
        .from("match_players")
  ...
  for (const p of activePlayers ?? []) {
    activeCountMap.set(p.match_id, (activeCountMap.get(p.match_id) ?? 0) + 1);
  }
      ...
      <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--mhl-muted)]">
            Partidos donde fuiste designado por la administración para registrar asistencia, cobros y resultados.
          </p>
        </div>
        ...
        {matchesList.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-[var(--mhl-border)] bg-[var(--mhl-panel)] p-10 text-center text-sm text-[var(--mhl-muted)]">
            ...
            <p className="mt-1 text-xs">
              Cuando un administrador te asigne como planillero principal de un partido, aparecerá aquí para operar la planilla.
            </p>
          </div>
        ) : (
              ...
              >
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    <span className={`rounded-lg border px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider ${style.bg}`}>
                      {style.label}
                    </span>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-xs text-[var(--mhl-muted)]">
                    <span>Jugadores: <strong className="text-[var(--mhl-green)]">{count} / {match.max_players ?? "—"}</strong></span>
                    <span>Tarifa: <strong className="text-[var(--mhl-text)]">${match.player_price?.toLocaleString("es-AR") ?? "0"}</strong></span>
                    {isCompleted && (
                ...
                <div>
                  <Link
                    ...
                    className={`block w-full text-center sm:w-auto rounded-xl px-5 py-3 text-xs font-black uppercase tracking-wider transition ${
                      isInProgress
                        ? "bg-purple-500 text-white hover:bg-purple-600 shadow-lg"
                        : "bg-[var(--mhl-yellow)] text-[#080b0a] hover:brightness-110"
                    }`}
                  >
                    {isInProgress ? "Planilla en Vivo &rarr;" : isCompleted ? "Ver Planilla Cerrada &rarr;" : "Abrir Planilla &rarr;"}
                  </Link>
                </div>
              </article>
            );
